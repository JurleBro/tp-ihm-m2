...
public class EntityManager implements IWatchableDirectoryObserver {
	...
	public void subscribeToDb(IDatabaseObserver observer) {
		this.mDatabase.addObserver(observer);
	}

	public User getUser(UUID uuid) {
		return this.mUserMap.get(uuid);
	}

	public boolean getUserExist(String tag) {
		return this.mUserMap.values().stream().anyMatch(u -> u.getUserTag().equals(tag));
	}

	public User getUserByName(String login) {
		return this.mUserMap.values().stream().filter(u -> u.getName().equals(login)).findFirst().orElse(null);
	}
}
